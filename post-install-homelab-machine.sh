...
REPO_URL=${6:-"git@github.com:primalivet/homelab.git"}
...
# Step 5: Clone the homelab repository
echo "[5/7] Cloning homelab repository..."
ssh gustaf@$MACHINE_IP "rm -rf ~/homelab && git clone $REPO_URL ~/homelab"
...
ssh -t gustaf@$MACHINE_IP "mkdir -p ~/homelab/machines/$MACHINE_NAME && sudo cp /etc/nixos/hardware-configuration.nix ~/homelab/machines/$MACHINE_NAME/ && sudo chown gustaf:users ~/homelab/machines/$MACHINE_NAME/hardware-configuration.nix"

# Step 7: Commit and push changes
echo "[7/7] Committing and pushing changes..."
ssh gustaf@$MACHINE_IP "cd ~/homelab && git add machines/$MACHINE_NAME/hardware-configuration.nix && git commit -m 'update $MACHINE_NAME hardware configuration' && git push origin main"

echo
echo "Post-installation setup complete!"
echo "The hardware configuration for $MACHINE_NAME has been added to your homelab repository."
